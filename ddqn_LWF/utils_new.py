...
import torch
from .prioritized_memory_new import Transition
...
def hdf5_to_memory(f, memory):
    for key in f.keys():
        ...
        transition = Transition(
            color, depth, pixel_index, reward, next_color, next_depth, is_empty, qmap)
...
def plot_figures(tool, color, depth, show=False, save=False, ros=False):
    ...
    return [angle, positions[1], positions[0]], Max, tool_cmap[max_.index(Max)], max_.index(Max)
