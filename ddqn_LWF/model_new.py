...
class reinforcement_net_new(nn.Module):
    def __init__(self, use_cuda, num_rotations=4):
        super(reinforcement_net_new, self).__init__()
        ...
        if is_volatile:  # For choosing action
            ...
            for rotate_idx in range(self.num_rotations):
                ...
                with torch.no_grad():
                    ...
                    # ========================= new task ========================= 
                    if rotate_idx == 2:
                        affordance_new = self.value_net_new(affordance)
                        output_prob_new.append(rotate_featuremap(
                            affordance_new, -theta, self.use_cuda))
                    # ========================= new task =========================
            ...
            return output_prob, output_prob_new
            ...
            if clear_grad:
                affordance.detach()
                affordance_new.detach()

            self.output_prob = rotate_featuremap(
                affordance, -theta, self.use_cuda)

            # ========================= new task =========================
            self.output_prob_new = rotate_featuremap(
                affordance_new, -theta, self.use_cuda)
            # ========================= new task =========================

        return self.output_prob, self.output_prob_new
