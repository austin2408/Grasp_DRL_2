...
from .prioritized_memory_new import Memory, Transition
from .utils_new import preprocessing, sample_data, get_action_info, hdf5_to_memory, postProcessing
...
class Agent():
    ...
    def load_pretrained_value_net(self, model_path: str) -> None:
        print('[agent] load model ', model_path)
        self.trainer.behavior_net.value_net.load_state_dict(torch.load(model_path))
        self.trainer.target_net.load_state_dict(
            self.trainer.behavior_net.state_dict())
    ...
    def train(self) -> dict:
        ...
        for j in range(len(mini_batch)):
            ...
            loss_ = self.trainer.backprop(
                color, depth, pixel_index, int(td_target), int(is_weight[j]), self.args.mini_batch_size, qmap, j == 0, j == int(len(mini_batch)-1))
        ...
        for j in range(len(mini_batch)):
            ...
            new_value = self.trainer.forward(color, depth, is_volatile=False, specific_rotation=2, clear_grad=True)[
                0, 0, pixel_index[0], pixel_index[1]]
